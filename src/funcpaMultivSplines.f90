...
! Copy spline coefficients
    ...
    the4=0.d0
    do i=1,nzloco+2
        the1(i-3)=dexp(bh(i))**2.0d0
    end do
    do i=1,nzdc+2
        the2(i-3)=dexp(bh(nzloco+2+i))**2.0d0
    end do
!    do i=1,nzmeta+2
!        the3(i-3)=dexp(bh(nzloco+nzdc+4+i))**2.0d0
!    end do
    do i=1,nzdc+2
        the4(i-3)=dexp(bh(nzloco+nzdc+4+i))**2.0d0
    end do

! One Random Effect Model
    if(typeJoint.eq.0)then
        ! b = c(splinesR1, splinesT1, splinesR2, splinesT2, theta, alpha1, alpha2, coef...)
        theta = dexp(bh(np-nva-2))**2.0d0! log_e random effect sd ---> varaince
